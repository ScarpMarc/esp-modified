...
#define DRV_NAME	"mac_vivado"

/* <<--regs-->> */
#define MAC_MAC_N_REG 0x48
#define MAC_MAC_LEN_REG 0x44
#define MAC_MAC_VEC_REG 0x40

struct mac_vivado_device {
...
static inline struct mac_vivado_device *to_mac(struct esp_device *esp)
{
	return container_of(esp, struct mac_vivado_device, esp);
}

static void mac_prep_xfer(struct esp_device *esp, void *arg)
{
	struct mac_vivado_access *a = arg;
...
{
	/* struct mac_vivado_device *mac = to_mac(esp); */
	/* struct mac_vivado_access *a = arg; */
...
static struct esp_driver mac_driver = {
	...
	.ioctl_cm	= MAC_VIVADO_IOC_ACCESS,
	.arg_size	= sizeof(struct mac_vivado_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("mac_vivado driver");
